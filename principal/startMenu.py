...
class RegisterWindow(FloatLayout):
    def __init__(self, init_page):
        ...
        self.password2 = TextInput(password=True, size_hint=(.4, .05),  pos_hint={'x':.5, 'y':.4})
    ...
    def btn_registrarse(self,obj):
        if(self.password.text == self.password2.text):
            if (len(self.password.text)>=8 and len(self.user.text)>0):
                

                if(self.contrasenas_manager.save_new(self.user.text, self.password.text)):
    ...
    def Fallo_longitud(self):
        content = Button(text='Aceptar', size_hint=(0.5, 0.5),font_size= 20)
        pop = Popup(title='La contraseña debe ser de más de 8 caracteres',
                content=content,
                title_align = 'center',
                title_size = '20',
                auto_dismiss=False,
